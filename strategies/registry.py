...
_STRATEGIES = {
    ...
    "Mean Reversion V1": StrategyDefinition(
        name="Mean Reversion V1",
        analyze=mean_reversion_v1.analyze,
        rank_key=mean_reversion_v1.rank_key,
        parameters=mean_reversion_v1.PARAMETERS,
        required_history_days=mean_reversion_v1.REQUIRED_HISTORY_DAYS,
    ),
}


def invoke_analyze(analyze, ticker, data, benchmark_data=None):
    """Call analyze(), passing `benchmark_data` only when the function accepts it.

    Existing strategies keep the (ticker, data) contract. Relative Strength
    Momentum V1 receives already-loaded benchmark history from the caller.
    This helper lives in the registry so strategy modules never import the
    engine (no circular imports).
    """
    try:
...
def extra_strategy_benchmark_tickers():
    """Tickers that registered strategies need as context, not as constituents."""
    return sorted(
        {
            definition.benchmark_ticker
            for definition in _STRATEGIES.values()
