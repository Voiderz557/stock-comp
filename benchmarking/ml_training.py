...
def train_ml_model_for_period(
    ...
    period is too close to the start of available history).
    """
    ...
    pipeline = MODEL_BUILDERS[model_name](feature_columns, categorical_columns)
    X = supervised[feature_columns + categorical_columns]
    y = supervised[target_column].astype(bool)
    pipeline.fit(X, y)
    ...
    return TrainedMLFold(
        ...
        validation_rows=0,  # filled in by the runner once the period simulation completes
