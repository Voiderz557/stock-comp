...
def _extend_price_history(downloaded_data, tickers, price_history_start_date, evaluation_start_date):
    ...
    *membership* lookups that are unsupported that far back, not prices.
    ...
    extra_data, _ = load_market_data(
        tickers, price_history_start_date, evaluation_start_date - pd.Timedelta(days=1)
    )
...
def run_benchmark(
    ...
    if price_data is None:
        ...
        downloaded_data, _ = download_backtest_data(
            evaluation_start_date, evaluation_end_date, benchmark, overall_required_history_days
        )
        ...
        downloaded_data = _extend_price_history(
            downloaded_data, extension_tickers, price_history_start_date, evaluation_start_date
        )
    else:
    ...
    for test_number, (period_start, period_end) in enumerate(periods, start=1):
        if progress_callback:
            progress_callback(test_number, len(periods))
        ...
        for model_name in ml_models:
            try:
                trained_fold = train_ml_model_for_period(
                    model_name,
                    period_start,
                    downloaded_data,
                    universe=universe,
    ...
    config = {
        ...
        "leakage_audit_is_valid": leakage_audit["Is Valid"],
        "benchmark_valid": leakage_audit["Is Valid"],
    }
